...
# Resolved from the central settings surface (amlguard.settings), so an endpoint or the
...
from amlguard.roster import Roster, roster_from_narratives, roster_from_parties
from amlguard.scopes import ProtectionScope
...
def protect_narratives(
    ...
    workers = max(1, int(os.getenv("AMLGUARD_DISCOVERY_WORKERS", "4")))
    if workers == 1:
        per_document = [

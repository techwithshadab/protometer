...
`docker/observability/mlflow/store/mlflow.db` (see DEFAULT_TRACKING_DIR / AMLGUARD_MLFLOW_STORE_DIR) -
...
# AMLGUARD_MLFLOW_STORE_DIR for a relocated or remote layout.
DEFAULT_TRACKING_DIR = Path(
    os.getenv("AMLGUARD_MLFLOW_STORE_DIR")
...
class Tracker:
    ...
    def __init__(
        ...
        self.enabled = enabled and os.getenv("AMLGUARD_NO_TRACKING") != "1"
        self._mlflow = None
    ...
    def log_model(self, model: Any, name: str, input_example: Any) -> str | None:
        ...
        try:
            ...
            info = self._mlflow.sklearn.log_model(
                ...
                registered_model_name=f"amlguard-{name}",
            )
    ...
    def govern_model(self, name: str, version: str, tags: dict[str, str],
        ...
        gets an alias (`champion` by default) that `models:/amlguard-<scope>@champion`

...
from protometer.eval.tasks import Checkpoint, Scoring, Stratum
...
# JUDGE_SYSTEM moved to config/prompts/protometer-judge-system.txt (loaded via observability.managed_prompt).
...
def _score_judge(checkpoint: Checkpoint, actual: Any, judge: Any) -> tuple[bool, str]:
    ...
    from protometer.llm import extract_json
    ...
    judge_system = managed_prompt("protometer-judge-system", project=domain_project("aml"))
    try:

...
def ensure_ollama_model(model_id: str, auto_pull: bool = False,
    ...
    the UI calls this with `auto_pull` from PROTOMETER_AUTO_PULL_MODEL; the Make target calls it True."""
    if not ollama_reachable():
        return False
...
class AnthropicProvider:
    ...
    def __init__(self) -> None:
        ...
        # default. Overridable via PROTOMETER_LLM_TIMEOUT for a batch run that wants more slack.
        self._client = anthropic.Anthropic(
            timeout=float(os.getenv("PROTOMETER_LLM_TIMEOUT", "120"))
        )
...
class OpenAIProvider:
    ...
    def __init__(self) -> None:
        ...
        self._client = OpenAI(timeout=float(os.getenv("PROTOMETER_LLM_TIMEOUT", "120")))
...
class LLMClient:
    ...
    max_spend_usd: float = float(os.getenv("PROTOMETER_MAX_SPEND_USD", "5.0"))
    ...
    # Optional Langfuse PROJECT this client's generations trace into (an Protometer domain's own
    ...
    def __post_init__(self) -> None:
        requested = self.model or os.getenv("PROTOMETER_MODEL") or self.registry.default
    ...
    def complete(self, system: str, prompt: str, max_tokens: int | None = None) -> str:
        ...
        if cached is not None:
            ...
            from protometer.observability import record_generation

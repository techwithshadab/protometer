"""Live-serving operational metrics for the Protometer chat API, exposed for Prometheus to SCRAPE.
...
Same posture as `metrics_export` and `observability`: entirely optional. `PROTOMETER_NO_METRICS=1` or a
...
def _disabled() -> bool:
    return os.getenv("PROTOMETER_NO_METRICS") == "1"
...
def _metrics() -> dict | None:
    ...
    m = {
        ...
        "turns": Counter(
            "protometer_serving_turns_total", "Live chat turns served",
            ["domain", "role", "outcome"],
        ),
        # End-to-end turn latency (protect -> reason -> egress -> reveal).
        "latency": Histogram(
            "protometer_serving_turn_latency_seconds", "End-to-end live turn latency",
        ...
        "entities_protected": Counter(
            "protometer_serving_entities_protected_total", "PII entities tokenized on inbound turns",
            ["domain"],
        ),
        "revealed": Counter(
            "protometer_serving_revealed_total", "Identifiers re-identified for the entitled role",
            ["domain", "role"],
        ),
        "egress_blocks": Counter(
            "protometer_serving_egress_blocks_total", "Replies withheld by the egress guard",
            ["domain"],
        ),
        "canary_hits": Counter(
            "protometer_serving_canary_hits_total", "Reveals that tripped the canary tripwire",
            ["domain"],
        ),
        "out_of_scope": Counter(
            "protometer_serving_out_of_scope_total", "Tokens withheld by scope-bound reveal",
            ["domain"],
        ),
        "errors": Counter(
            "protometer_serving_errors_total", "Turns that ended in an error, by kind",
            ["domain", "kind"],
        ),
        # Model / token usage — cost + efficiency signal (Langfuse has per-generation detail; this is
        # the aggregate time-series view).
        "llm_calls": Counter(
            "protometer_serving_llm_calls_total", "LLM generation calls on the serving path",
            ["domain", "model"],
        ),
        "tokens": Counter(
            "protometer_serving_llm_tokens_total", "LLM tokens on the serving path, by direction",
...
def record_turn(
    domain: str,
    role: str,
    result,  # protometer.serving.TurnResult (duck-typed to avoid an import cycle)
    *,
    model: str = "",
    input_tokens: int = 0,
    output_tokens: int = 0,

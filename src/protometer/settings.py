...
def discovery_url() -> str:
    ...
    return _get("PROTOMETER_DISCOVERY_URL",
...
def discovery_threshold() -> float:
    ...
    return _get_float("PROTOMETER_DISCOVERY_THRESHOLD", 0.6)


def discovery_workers() -> int:
    return _get_int("PROTOMETER_DISCOVERY_WORKERS", 4)
...
def guardrail_url() -> str:
    ...
    return _get("PROTOMETER_GUARDRAIL_URL",
...
def anonymization_url() -> str:
    ...
    return _get("PROTOMETER_ANONYMIZATION_URL", "http://localhost:6004/pty/anonymization/v3")
...
def synthetic_url() -> str:
    ...
    compose overrides this to pty-synthetic:8000). Note :8000 on the host is the Protometer UI — the
    shared tier deliberately remaps this service off :8000 to :6005 to avoid that collision."""
    return _get("PROTOMETER_SYNTHETIC_URL", "http://localhost:6005/pty/syntheticdata/v2")
...
def ui_api_base() -> str:
    ...
    return _get("PROTOMETER_UI_API_BASE", "http://localhost:8000")
...
def ui_api_token() -> str | None:
    ...
    """
    return os.getenv("PROTOMETER_UI_API_TOKEN") or None


def ui_role_tokens() -> dict[str, str]:
    """Optional map of API token -> role name, so the CALLER's role is proven by their token
    rather than chosen freely in the request body. Format: ``PROTOMETER_UI_ROLE_TOKENS`` =
    ...
    """
    raw = os.getenv("PROTOMETER_UI_ROLE_TOKENS") or ""
    out: dict[str, str] = {}
...
def ui_max_turns() -> int:
    ...
    """
    return _get_int("PROTOMETER_UI_MAX_TURNS", 200)
...
def _in_container() -> bool:
    ...
    if os.getenv("PROTOMETER_IN_CONTAINER") == "1":
        return True
...
def default_model() -> str | None:
    return os.getenv("PROTOMETER_MODEL")


def max_spend_usd() -> float:
    return _get_float("PROTOMETER_MAX_SPEND_USD", 5.0)
...
def ui_model() -> str | None:
    ...
    return os.getenv("PROTOMETER_UI_MODEL") or None
...
def local_model() -> str:
    ...
    return _get("PROTOMETER_LOCAL_MODEL", "llama3.2")
...
def hosted_ui_model() -> str:
    ...
    return _get("PROTOMETER_HOSTED_MODEL", "bedrock-sonnet-5")
...
def auto_pull_model() -> bool:
    ...
    return _get_bool("PROTOMETER_AUTO_PULL_MODEL", False)
...
# --- Observability ----------------------------------------------------------------------------
def mlflow_uri() -> str:
    return _get("PROTOMETER_MLFLOW_URI", "http://localhost:5001")
...
def mlflow_store_dir() -> str | None:
    ...
    return os.getenv("PROTOMETER_MLFLOW_STORE_DIR") or None
...
def postgres_url() -> str:
    ...
    return _get("PROTOMETER_POSTGRES_URL", "postgresql://protometer:protometer@localhost:8001/protometer")
...
def pushgateway() -> str:
    ...
    return _get("PROTOMETER_PUSHGATEWAY", "localhost:5004")
...
def prometheus_query_url() -> str:
    ...
    return _get("PROTOMETER_PROMETHEUS_URL", "http://localhost:5003")


def log_level() -> str:
    return _get("PROTOMETER_LOG_LEVEL", "INFO")


# --- Telemetry kill switches (all default OFF -> telemetry ON) ---------------------------------
def no_tracking() -> bool:
    return _flag("PROTOMETER_NO_TRACKING")


def no_tracing() -> bool:
    return _flag("PROTOMETER_NO_TRACING")


def no_metrics() -> bool:
    return _flag("PROTOMETER_NO_METRICS")
...
KNOWN_SETTINGS: tuple[tuple[str, bool], ...] = (
    ...
    ("PROTOMETER_DISCOVERY_URL", True),
    ("PROTOMETER_DISCOVERY_THRESHOLD", True),
    ("PROTOMETER_DISCOVERY_WORKERS", True),
    ("PROTOMETER_GUARDRAIL_URL", True),
    ("PROTOMETER_ANONYMIZATION_URL", True),
    ("PROTOMETER_SYNTHETIC_URL", True),
    ("PROTOMETER_PROMETHEUS_URL", True),
    ("PROTOMETER_UI_API_BASE", True),
    ("PROTOMETER_UI_API_TOKEN", True),
    ("PROTOMETER_UI_ROLE_TOKENS", True),
    ("PROTOMETER_UI_MAX_TURNS", True),
    ("OLLAMA_URL", True),
    ("PROTOMETER_IN_CONTAINER", True),
    ("PROTOMETER_MODEL", True),
    ("PROTOMETER_UI_MODEL", True),
    ("PROTOMETER_LOCAL_MODEL", True),
    ("PROTOMETER_HOSTED_MODEL", True),
    ("PROTOMETER_AUTO_PULL_MODEL", True),
    ("PROTOMETER_MAX_SPEND_USD", True),
    ("PROTOMETER_LLM_TIMEOUT", True),
    ("ANTHROPIC_API_KEY", True),
    ("OPENAI_API_KEY", True),
    ("AWS_REGION", True),
    ("AWS_DEFAULT_REGION", True),
    ("PROTOMETER_MLFLOW_URI", True),
    ("PROTOMETER_MLFLOW_STORE_DIR", True),
    ("PROTOMETER_POSTGRES_URL", True),
    ("LANGFUSE_HOST", True),
    ("LANGFUSE_PUBLIC_KEY", True),
    ("LANGFUSE_SECRET_KEY", True),
    ("LANGFUSE_TIMEOUT", True),
    ("PROTOMETER_PUSHGATEWAY", True),
    ("PROTOMETER_LOG_LEVEL", True),
    ("PROTOMETER_NO_TRACKING", True),
    ("PROTOMETER_NO_TRACING", True),
    ("PROTOMETER_NO_METRICS", True),
    ("DOTENV_PATH", True),
)

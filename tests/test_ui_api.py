...
def test_chat_turn_requires_token_when_configured(monkeypatch):
    mod = _load_app(monkeypatch, PROTOMETER_UI_API_TOKEN="s3cret", PROTOMETER_NO_TRACKING="1",
...
def test_chat_turn_open_on_loopback_when_no_token(monkeypatch):
    mod = _load_app(monkeypatch, PROTOMETER_NO_TRACKING="1", PROTOMETER_NO_TRACING="1")
...
def test_turn_ceiling_rejects_past_cap(monkeypatch):
    mod = _load_app(monkeypatch, PROTOMETER_UI_MAX_TURNS="2", PROTOMETER_NO_TRACKING="1",
                    PROTOMETER_NO_TRACING="1")
    mod._charge_turn()
    mod._charge_turn()
...
def test_turn_ceiling_disabled_when_zero(monkeypatch):
    mod = _load_app(monkeypatch, PROTOMETER_UI_MAX_TURNS="0", PROTOMETER_NO_TRACKING="1",
                    PROTOMETER_NO_TRACING="1")
    for _ in range(50):
...
def test_corpus_parties_503_when_postgres_down(monkeypatch):
    ...
    TestClient = fastapi_testclient.TestClient
    mod = _load_app(monkeypatch, PROTOMETER_NO_TRACKING="1", PROTOMETER_NO_TRACING="1")
    from protometer import db as _db
...
def test_corpus_parties_requires_token_when_configured(monkeypatch):
    ...
    mod = _load_app(monkeypatch, PROTOMETER_UI_API_TOKEN="s3cret",
                    PROTOMETER_NO_TRACKING="1", PROTOMETER_NO_TRACING="1")
    c = TestClient(mod.app)
...
def test_resolve_ui_model_prefers_hosted_when_aws_present(monkeypatch):
    ...
    monkeypatch.delenv("PROTOMETER_UI_MODEL", raising=False)
...
def test_resolve_ui_model_honours_explicit_override(monkeypatch):
    mod = _load_app(monkeypatch, PROTOMETER_UI_MODEL="gpt-4o-mini",
                    PROTOMETER_NO_TRACKING="1", PROTOMETER_NO_TRACING="1")
    from protometer import settings
    ...
    assert "PROTOMETER_UI_MODEL=gpt-4o-mini" in reason


def test_resolve_ui_model_flags_invalid_override(monkeypatch):
    mod = _load_app(monkeypatch, PROTOMETER_UI_MODEL="no-such-model",
                    PROTOMETER_NO_TRACKING="1", PROTOMETER_NO_TRACING="1")
    model, reason = mod.resolve_ui_model()
